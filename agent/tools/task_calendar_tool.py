...
class TaskCalendarTool:
    ...
    The main lesson from this project is that tasks should not be checked only
    at the last minute. Fixed visits, full off-days and no-action windows need a
    rolling calendar so profitable orders can be rejected before they consume
    reserved time. The opposite is also true: tasks that are far in the future
    should not suppress market orders too early, otherwise the agent earns very
    little while still looking "preference safe".
    """
    ...
    def calendar_report(
        ...
        *,
        ...
        # Each sub-plan contributes either an immediate action or a finish-time
        # deadline for cargo candidates. This lets the market scorer keep
        # earning money until a task is actually close enough to matter.
